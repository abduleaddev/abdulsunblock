...
if (empty($url)) {
    if (empty($startURL)) {
      die("<html>
      <head>
      <title>Thank you, Abdul!</title>
      </head><body>
      <h1>abduls unblock!</h1>
      if this site goes down ever lmk and ill fix it   :
      ...
      <br />
      <br />
      <form onsubmit=\"if (document.getElementById('site').value) { window.location.href='" . PROXY_PREFIX . "' + document.getElementById('site').value; return false; } else { window.location.href='" . PROXY_PREFIX . $landingExampleURL . "'; return false; }\" autocomplete=\"off\"><input id=\"site\" type=\"text\" size=\"50\" /><input type=\"submit\" value=\"thanks abdul!\" /></form></body></html>");
    } else {
      $url = $startURL;
    }
